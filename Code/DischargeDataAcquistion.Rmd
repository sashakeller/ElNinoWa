...
TulalipDischarge$WaterYear <- as.numeric(as.character(TulalipDischarge$WaterYear))
TulalipENSO<-left_join(TulalipProccessed, enso_processed)
...
SkokomishProccessed <- SkokomishDischarge %>%
  mutate(Baseflow = baseflow(Discharge), 
         ...
         Month = month(Date),
         Discharge_acftmo = sum(Discharge)*1.98347)
SkokomishDischarge$WaterYear <- as.numeric(as.character(SkokomishDischarge$WaterYear))
SkokomishENSO<-left_join(SkokomishProccessed, enso_processed)
...
DuckabushENSO<-left_join(DuckProccessed, enso_processed)
#Puyallup
...
PuyallupENSO<-left_join(PuyallupProccessed, enso_processed)
